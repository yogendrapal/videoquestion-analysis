...
if test:
    ...
    stoppath = os.getcwd()+"/python-src/"+"SmartStoplist.txt"  #SMART stoplist misses some of the lower-scoring keywords in Figure 1.5, which means that the top 1/3 cuts off one of the 4.0 score words in Table 1.1
    ...
    rake = Rake(os.getcwd()+"/python-src/"+"SmartStoplist.txt")
    keywords = rake.run(text)
    print(keywords)
    print(type(keywords[0]))
    l=[]
    for i in sortedKeywords[0:int(totalKeywords / 3)]:
    	l.append(i[0])
    print(l)
    d=dict()
    d['keyword']=l
    ...
    print(os.getcwd())
    filename=sys.argv[1]
    name1=os.getcwd()+"/json-files/"+Video+".json"
    print (filename)
    print(Video)
